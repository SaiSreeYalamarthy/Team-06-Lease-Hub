...
const SharedaccomadationPage = () => {
  ...
  const fetchPropertiesAndUpdateWishlist = async () => {
    try {
      const [propertiesResponse, wishlistStatus] = await Promise.all([
        fetch('http://localhost:9000/properties?type=sharedaccommodation'),
        fetchWishlistStatus(),
      ]);
  ...
  const fetchProperties = async () => {
    ...
    try {
      const response = await fetch('http://localhost:9000/properties?type=sharedaccommodation');
      if (response.ok) {
  ...
  const handleCompareClick = () => {
    ...
    return (
      <div style={containerStyles}>
        <h1 style={headerStyles}>Welcome to the Shared Accomadation Page</h1>
...
export default SharedaccomadationPage;