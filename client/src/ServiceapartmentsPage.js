...
const ServiceApartmentPage = () => {
  ...
  const [serviceapartmentProperties, setServiceApartmentProperties] = useState([]);
  ...
  const fetchPropertiesAndUpdateWishlist = async () => {
    try {
      const [propertiesResponse, wishlistStatus] = await Promise.all([
        fetch('http://localhost:9000/properties?type=serviceapartments'),
        fetchWishlistStatus(),
      ]);
  ...
  const fetchProperties = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:9000/properties?type=serviceapartments');
      ...
      if (response.ok) {
        ...
        setServiceApartmentProperties(updatedProperties);
  ...
  return (
    <div style={containerStyles}>
      <h1 style={headerStyles}>Welcome to the service apartment Page</h1>
...
export default ServiceApartmentPage;