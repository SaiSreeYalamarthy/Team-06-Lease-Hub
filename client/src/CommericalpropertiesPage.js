...
const CommercialPropertiesPage = () => {
  ...
  const fetchPropertiesAndUpdateWishlist = async () => {
    try {
      const [propertiesResponse, wishlistStatus] = await Promise.all([
        fetch('http://localhost:9000/properties?type=commercial'),
        fetchWishlistStatus(),
      ]);
  ...
  const fetchProperties = async () => {
    ...
    try {
      const response = await fetch('http://localhost:9000/properties?type=commercial');
      if (response.ok) {
  ...
  const handleCompareClick = () => {
    ...
    return (
      <div style={containerStyles}>
        <h1 style={headerStyles}>Welcome to the Commercial Properties Page</h1>
