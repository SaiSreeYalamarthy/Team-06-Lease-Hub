...
const BuyPage = () => {
  ...
  const fetchPropertiesAndUpdateWishlist = async () => {
    try {
      const [propertiesResponse, wishlistStatus] = await Promise.all([
        fetch('http://localhost:9000/properties?type=buy'),
        fetchWishlistStatus(),
      ]);
  ...
  const fetchProperties = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:9000/properties?type=buy');
      ...
      if (response.ok) {
        ...
        setBuyProperties(updatedProperties);
  ...
  //   if (!property || !property.image) return <p>No image available</p>;

  //   const base64Url = `data:${property.image.contentType};base64,${property.image.data}`;
  //   return <img src={base64Url} alt="Property" />;
  // };
  const renderImage = (property) => {
  ...
  return (
    <div style={containerStyles}>
      <h1 style={headerStyles}>Welcome to the Buy Page</h1>
  ...
  {filteredProperties.map((property) => (
    property ? ( // Check if the property is not null
      ...
      // Display a message if the property is null
